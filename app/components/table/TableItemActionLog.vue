...
import { itemActionLogQuery } from '~/utils/queries/inventory/items'
...
const tableData = computed((): ItemActionLogRow[] =>
  [...sourceEntries.value]
...
const columns: ColumnDef<ItemActionLogRow>[] = [
  { header: 'Action', accessorKey: 'actionLabel', meta: { una: { tableHead: TABLE_HEAD_STYLE, tableCell: 'align-top' } } },
  { header: 'User', accessorKey: 'userName', meta: { una: { tableHead: TABLE_HEAD_STYLE, tableCell: 'align-top' } } },
  { header: 'When', accessorKey: 'timestamp', meta: { una: { tableHead: TABLE_HEAD_STYLE, tableCell: 'align-top' } } },
  { header: 'Flag', accessorKey: 'flag', meta: { una: { tableHead: TABLE_HEAD_STYLE, tableCell: 'align-top' } } }
]

const pagination = ref({ pageSize: 10, pageIndex: 0 })
const expanded = ref<Record<string, boolean>>({})
const table = useTemplateRef<Table<ItemActionLogRow>>('table')
const canLoadOlder = computed(() => !fullLogLoaded.value && props.entries.length >= props.recentCap)

function rowHasDetails(row: ItemActionLogRow): boolean {
