...
import { alterItem as useAlterItemsMutation } from '~/utils/mutations/inventory/items'
...
// Actions permitted for the WHOLE selection: the intersection of each item's
...
const commonAllowedActions = computed<Set<InventoryActionType>>(() => {
  if (props.items.length === 0) return new Set<InventoryActionType>()
  let common: InventoryActionType[] | null = null
  for (const item of props.items) {
    const allowed = allowedActionsForStatus(item.status).filter(isAlterAction)
    common = common === null
      ? [...allowed]
...
// Flags currently present across the selected items — you can only remove a flag that exists.
const availableFlags = computed<InventoryFlagType[]>(() => {
  const seen = new Set<InventoryFlagType>()
  for (const item of props.items) {
    for (const flag of item.activeFlags ?? []) seen.add(flag.kind)
  }
  return (Object.keys(FLAG_META) as InventoryFlagType[]).filter(flag => seen.has(flag))
})

// Show only actions allowed for every selected item; hide "unflag" when none carry a flag.
...
const { mutateAsync: alterItemsAsync, isLoading } = useAlterItemsMutation()

async function handleAlter() {
  if (count.value === 0) return

  try {
    await alterItemsAsync({
...
<template>
  <NDialog
    :open="isOpen"
    title="Apply action to items"
    description="Perform a lifecycle action on every selected item at once."
  ...
  >
    ...
    <div class="p-4 space-y-4">
      <p class="text-muted text-sm">
        Applying to <span class="font-semibold">{{ count }}</span> item{{ count === 1 ? '' : 's' }}.
      </p>
      ...
      <NAlert
        ...
        description="The selected items have no lifecycle action in common for their current statuses."
        icon
      />
      ...
      >
        <p class="text-sm">
          Disposing frees each item's slot in its parent and marks it
      ...
      >
        <NInput
          ...
          type="textarea"
          :rows="2"
          placeholder="Optional — appended to each item's action log"
        />
      </NFormField>
    </div>
