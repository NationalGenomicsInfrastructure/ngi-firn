    ...
    <div class="mx-auto max-w-xl rounded-md">
      <div class="border-2 border-primary-600 dark:border-primary-400 rounded-md p-4 text-center text-sm flex flex-col items-center space-y-4 bg-primary-50 dark:bg-primary-700">
        ...
        <template v-if="message.title || message.body">
          <h1
            v-if="message.title"
            class="scroll-m-20 text-center font-display font-semibold text-primary-600 dark:text-primary-400 tracking-tight text-display text-2xl lg:text-3xl"
          >
            {{ message.title }}
          </h1>
          <p
            v-if="message.body"
            class="text-lg text-center whitespace-pre-line"
          >
            {{ message.body }}
          </p>
        </template>
